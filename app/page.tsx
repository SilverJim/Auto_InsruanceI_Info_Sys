...
import { compareQuotes, mockQuoteProvider, type Quote } from "./services/quotes";
...
export default function Home() {
  ...
  const submit = async () => {
    setLoading(true);
    const results = await mockQuoteProvider.getQuotes({ ...form, consent });
    setQuotes(results);
    setLoading(false);
  };
    ...
    {!quotes ? <div className="app-shell">
      ...
      <section className="form-panel">
        ...
        {step === 4 && <section className="form-card review-card">
          <p className="eyebrow">READY TO COMPARE</p><h2>Review what will happen.</h2><p className="lede">This demo uses fictional provider responses. No information leaves this application.</p>
          <div className="summary-grid"><div><small>DRIVER</small><b>{form.firstName || "Your name"} {form.lastName}</b><span>{form.licenceClass} licence · Since {form.firstLicensed || "—"}</span></div><div><small>VEHICLE</small><b>{form.vehicleYear} {form.make} {form.model}</b><span>{Number(form.annualKm).toLocaleString()} km · {form.use}</span></div><div><small>BENCHMARK</small><b>{form.liability} liability</b><span>{form.deductible} deductibles · DCPD</span></div><div><small>PROVIDERS</small><b>4 verified demo routes</b><span>3 exact quotes · 1 estimate</span></div></div>
          <div className="provider-row"><div className="provider-mini plum">P</div><div className="provider-mini navy">N</div><div className="provider-mini orange">S</div><div className="provider-mini green">M</div><p><b>Four distinct rate sources</b><span>Legal underwriter and licence status shown in results.</span></p></div>
          <div className="consent"><input id="demo-consent" type="checkbox" checked={consent} onChange={e => setConsent(e.target.checked)} aria-labelledby="demo-consent-label" /><span><label id="demo-consent-label" htmlFor="demo-consent"><b>I confirm this profile is accurate and approve this demo comparison.</b></label><small>In a live run, you will see the exact fields shared with each provider and may exclude any route.</small></span></div>
        </section>}
        <footer className="form-actions"><button className="back" onClick={() => setStep(Math.max(0, step - 1))} disabled={step === 0}>← Back</button>{step < 4 ? <button className="primary" onClick={() => setStep(step + 1)}>Continue <span>→</span></button> : <button className="primary" disabled={!consent || loading} onClick={submit}>{loading ? "Comparing securely…" : "Compare demo quotes"} <span>→</span></button>}</footer>
      </section>
    </div> : <section className="results-page">
      <div className="results-hero"><div><Mark tone="green">✓ Comparison complete</Mark><h1>Your coverage-first comparison.</h1><p>We found <b>3 exact quotes</b> across four verified demo rate sources. Prices are fictional and for product demonstration only.</p></div><div className="hero-score"><small>MARKET COMPLETION</small><strong>100%</strong><span>4 of 4 sources resolved</span></div></div>
      <div className="result-toolbar"><div className="tabs"><button className={filter === "all" ? "selected" : ""} onClick={() => setFilter("all")}>All results <span>4</span></button><button className={filter === "comparable" ? "selected" : ""} onClick={() => setFilter("comparable")}>Comparable only <span>2</span></button></div><button className="edit-profile" onClick={restart}>← Edit profile</button></div>
      <div className="results-layout"><div className="quote-list">
        {visibleQuotes.map((quote, index) => <article className={`quote-card ${index === 0 && quote.comparable ? "featured" : ""}`} key={quote.id}>
          ...
          <div className="quote-main"><div className={`provider-logo ${quote.color}`}>{quote.initials}</div><div className="provider-info"><div className="provider-title"><h3>{quote.brand}</h3><Mark tone={quote.verified ? "green" : "amber"}>{quote.verified ? "✓ Verified provider" : "Estimate only"}</Mark></div><p>Underwritten by {quote.underwriter}</p><small>Verified Aug 9, 2026 · Distinct source {quote.sourceId}</small></div><div className="price"><small>ANNUAL TOTAL</small><strong>${quote.annual.toLocaleString()}</strong><span>${quote.monthly}/month {quote.fee > 0 ? `+ $${quote.fee} financing` : "· no fee"}</span></div></div>
          <div className="coverage-row"><div><small>LIABILITY</small><b>{quote.liability}</b></div><div><small>DEDUCTIBLES</small><b>{quote.deductible}</b></div><div><small>DCPD</small><b>{quote.dcpd ? "Included" : "Not included"}</b></div><div><small>OPCF 44R</small><b>{quote.opcf44 ? "Included" : "Unknown"}</b></div><Mark tone={quote.comparable ? "blue" : "amber"}>{quote.comparable ? "Benchmark matched" : quote.statusLabel}</Mark></div>
      ...
      </div><aside className="ledger"><p className="eyebrow">COVERAGE LEDGER</p><h3>Every source accounted for.</h3><div className="ledger-stat"><strong>4</strong><span>Verified applicable<br/>rate sources</span></div><div className="ledger-lines"><p><i className="dot green-dot"/>2 comparable quotes <b>50%</b></p><p><i className="dot blue-dot"/>1 non-comparable quote <b>25%</b></p><p><i className="dot amber-dot"/>1 estimate only <b>25%</b></p><p><i className="dot grey-dot"/>0 unresolved <b>0%</b></p></div><div className="ledger-note"><span>i</span><p><b>Price isn’t the whole policy.</b><br/>Review differences and eligibility conditions before contacting a licensed representative.</p></div><button className="download">Download redacted report ↓</button></aside></div>
      <div className="demo-banner"><span>DEMO DATA</span><p>These quotes are fictional. The quote provider interface is ready for approved insurer, broker, aggregator, or manual-handoff adapters.</p></div>
    </section>}
